...
NumericVector distancia (NumericMatrix neuronas,NumericVector puntofijo, int k, int h){
...
int FindBMU(NumericMatrix datos,NumericMatrix neuronas,int indexEstimulo,int k, int h){
  NumericVector dist = distancia(neuronas, datos(indexEstimulo,_),k,h);
...
Rcpp::DataFrame myFunc(int k,int h,float alfaMAyor ,float AlfaMenor,
  ...
  for(int i = 0 ; i <nIteraciones ; i++){
    ...
    int mejorNeurona = FindBMU( datos, neuronas,nRandomr(0),k, h);
